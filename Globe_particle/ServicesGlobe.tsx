...
const ServicesGlobe: React.FC = () => {
  ...
  useEffect(() => {
    ...
    for(let i = 0; i < innerNoiseCount; i++) {
      ...
      const r = outerRadius * (0.3 + Math.random() * 0.5);
    ...
    const outerNoiseCount = 8000;
    for(let i = 0; i < outerNoiseCount; i++) {
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.random() * Math.PI * 2;
      const r = outerRadius * (1.05 + Math.random() * 0.1);
      const x = r * Math.cos(phi) * Math.sin(theta);
      const y = r * Math.cos(theta);
      const z = r * Math.sin(phi) * Math.sin(theta);
    ...
    for(let i = 0; i < wrapperCount; i++) {
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.random() * Math.PI * 2;
      const r = outerRadius * (1.15 + Math.random() * 0.1);
      const x = r * Math.cos(phi) * Math.sin(theta);
      const y = r * Math.cos(theta);
      const z = r * Math.sin(phi) * Math.sin(theta);
    ...
    // Additional Inner-Outer Noise Layer
    const innerOuterNoiseParticles: number[] = [];
    const innerOuterNoiseCount = 25000;
    for(let i = 0; i < innerOuterNoiseCount; i++) {
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.random() * Math.PI * 2;
      const r = outerRadius * (0.90 + Math.random() * 0.07);
      const x = r * Math.cos(phi) * Math.sin(theta);
      const y = r * Math.cos(theta);
      const z = r * Math.sin(phi) * Math.sin(theta);
    ...
    for(let i = 0; i < coreCount; i++) {
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.random() * Math.PI * 2;
      const r = outerRadius * (Math.random() * 0.2);
      const x = r * Math.cos(phi) * Math.sin(theta);
      const y = r * Math.cos(theta);
      const z = r * Math.sin(phi) * Math.sin(theta);
    ...
    [surfaceSystem, innerNoiseSystem, outerNoiseSystem, 
     innerOuterNoiseSystem, coreSystem, wrapperSystem].forEach(system => {
      system.rotation.z = Math.PI * (20/180);  // Set to 20 degrees
    });
